...
def extract_audio_features(path, mode='wav2vec'):
    ...
    if mode == 'wav2vec':
        cmd = f'python preprocess_utils/data_utils/nerf/asr.py --wav {path} --save_feats'
    else: # deepspeech
...
if __name__ == '__main__':
    ...
    parser.add_argument('base_dir', type=str, help="path to save directory")
    ...
    task = opt.task
    wav_path = os.path.join(base_dir, 'aud.wav') if task == -1 else opt.path
    # wav_path = './data/test/aud.wav'
